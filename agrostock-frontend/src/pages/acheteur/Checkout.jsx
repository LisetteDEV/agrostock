...
const Checkout = () => {
  ...
  const handleConfirm = async (e) => {
    ...
    if (
      ...
      (!ville.trim() || !quartier.trim() || !adresse.trim())
    ) {
      setSubmitError(
        "Veuillez remplir toutes les informations de livraison (ville, quartier, adresse).",
      );
      return;
    }
    ...
    if (!telephone.trim()) {
      setSubmitError(
        "Veuillez renseigner un numéro de contact pour la réception.",
      );
      return;
    }
    ...
    if (typePaiement === "mobile_money") {
      if (!numeroMobile.trim() || numeroMobile !== confirmNumero) {
        setSubmitError(
          "Le numero Mobile Money et sa confirmation doivent correspondre.",
        );
        return;
      }
    } else {
    ...
    const invalidGross = panier.find(
      (item) =>
        item.mode_achat === "gros" &&
        Number(item.quantite) < Number(item.quantite_min_gros || 10),
    );
    if (invalidGross) {
      setSubmitError(
        `Le produit ${invalidGross.nom} doit etre commande en gros avec minimum ${invalidGross.quantite_min_gros || 10}.`,
      );
      return;
    }
    ...
    if (panierPayload.length === 0) {
      setSubmitError(
        "Votre panier est invalide. Rafraichissez la page et reessayez.",
      );
      return;
    }
    ...
    try {
      ...
      // Afficher "Paiement Réussi" très brievement
    ...
    return (
      ...
      >
        <div
          ...
          style={{ ...premiumCardStyle, maxWidth: "500px", width: "90%" }}
        >
          <div
          ...
          <button
            ...
            className="btn btn-success rounded-pill px-5 py-3 fw-bold shadow-sm w-100"
  ...
  return (
    <div
      style={{
        background: "linear-gradient(135deg, #f8fbf8 0%, #edf2f0 100%)",
        minHeight: "100vh",
      }}
    >
      {/* Simulation Overlay */}
      {showSimulation && (
        ...
        >
          <div
            className="bg-white rounded-5 p-5 text-center shadow-lg"
            style={{
              maxWidth: "450px",
              width: "92%",
            ...
            {simulationState === "waiting" ? (
              <>
                <div className="position-relative d-inline-flex mb-4">
                  ...
                  <ShieldCheck
                    size={28}
                    className="text-success position-absolute top-50 start-50 translate-middle"
                  />
                </div>
                <h4 className="fw-bold mb-2 text-dark">Paiement en cours...</h4>
                <p className="text-muted mb-0 fs-6">
                  Veuillez patienter pendant le traitement sécurisé.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-muted mb-0 fs-6">Redirection en cours...</p>
              </>
            )}
          </div>
        </div>
      ...
      {successData ? (
        <div className="container py-5" style={{ paddingTop: "120px" }}>
          <div
            className="mx-auto"
            style={{ ...premiumCardStyle, maxWidth: "800px", padding: "3rem" }}
          >
            <div className="text-center mb-5">
              <div
                className="d-inline-flex align-items-center justify-content-center bg-success text-white rounded-circle shadow mb-4"
                style={{ width: "90px", height: "90px" }}
              >
                <CheckCircle2 size={46} />
              </div>
              <h2 className="fw-bolder" style={{ color: "#0f172a" }}>
                Commande validée !
              </h2>
            ...
            {(successData.bons_retrait || []).length > 0 && (
              <div className="bg-success bg-opacity-10 border border-success rounded-4 p-4 mb-5">
                <h5 className="fw-bold text-success mb-3 d-flex align-items-center gap-2">
                  <Lock size={20} /> Vos Bons de Retrait
                </h5>
                    ...
                    <div key={bon.commande_id} className="col-md-6">
                      <div className="bg-white rounded-3 p-3 shadow-sm h-100 border border-light">
                        <div className="text-muted small mb-1">
                          Commande #CMD-
                          {String(bon.commande_id).padStart(4, "0")}
                        </div>
                        ...
                        <div className="small text-muted mb-3">
                          <MapPin size={14} className="me-1" /> Zone:{" "}
                          {bon.zone_retrait || "-"}
                        </div>
                        <div className="d-flex flex-column gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => downloadBonImage(bon)}
                            className="btn btn-success btn-sm rounded-pill w-100 fw-medium shadow-sm"
                          >
                            Télécharger le Bon
                          </button>
                        </div>
            ...
            <div className="bg-light rounded-4 p-4 mb-5">
              ...
              {(successData.commandes_id || []).map((id) => (
                <div
                  key={id}
                  className="d-flex justify-content-between align-items-center bg-white p-3 rounded-3 mb-2 shadow-sm border border-light"
                >
            ...
            <div className="text-center">
              <button
                onClick={() => navigate("/dashboard-acheteur")}
                className="btn btn-dark rounded-pill px-5 py-3 fw-bold fs-5 shadow-lg w-100 w-md-auto"
        ...
        >
          <div className="d-flex align-items-center justify-content-between mb-5">
            <div>
              ...
              <p className="text-muted mb-0 fs-5" style={{ fontWeight: "500" }}>
                Réglez votre commande en toute sécurité.
              </p>
            </div>
            <div
              className="d-none d-md-flex align-items-center text-success bg-white px-4 py-3 rounded-pill shadow-sm fw-bold border border-success border-opacity-25"
          ...
          <form onSubmit={handleConfirm} className="row g-4">
            <div className="col-lg-7">
                ...
                <div className="d-flex align-items-center gap-3 mb-4 border-bottom pb-4">
                  ...
                  <div>
                    <h4
                      ...
                      style={{ color: "#0f3a23", letterSpacing: "-0.5px" }}
                    >
                      Logistique & Livraison
                    </h4>
                    <p className="text-muted small mb-0">
                      Définissez vos préférences de réception
                    </p>
                  </div>
                </div>
                ...
                <div className="row g-4">
                  <div className="col-12 mb-3">
                    <div className="d-flex gap-3 flex-column flex-md-row">
                      <label
                        className={`form-check flex-fill p-3 rounded-4 position-relative ${logistiqueMode === "livreur_propre" ? "bg-success bg-opacity-10" : "bg-white"}`}
                        style={{
                          cursor: "pointer",
                          transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
                          border:
                            logistiqueMode === "livreur_propre"
                              ? "2px solid #1ab273"
                              : "2px solid #e2e8f0",
                          transform:
                            logistiqueMode === "livreur_propre"
                          ...
                          boxShadow:
                            logistiqueMode === "livreur_propre"
                              ? "0 10px 20px -5px rgba(26,178,115,0.15)"
                              : "none",
                        }}
                      >
                        <div className="d-flex align-items-center">
                          <input
                            ...
                            type="radio"
                            value="livreur_propre"
                            checked={logistiqueMode === "livreur_propre"}
                            onChange={(e) => setLogistiqueMode(e.target.value)}
                          />
                          <div className="d-flex flex-column">
                            <span
                              className={`fw-bold fs-6 mb-1 ${logistiqueMode === "livreur_propre" ? "text-success" : "text-dark"}`}
                              style={{ letterSpacing: "-0.2px" }}
                            >
                              J'ai mon propre livreur
                            </span>
                            <span className="text-muted small fw-medium">
                              Aucun frais logistique appliqué
                            </span>
                          </div>
                        </div>
                        {logistiqueMode === "livreur_propre" && (
                          <CheckCircle2
                            size={24}
                            className="text-success position-absolute top-50 translate-middle-y end-0 me-4 opacity-50"
                          />
                        )}
                      </label>
                      <label
                        className={`form-check flex-fill p-3 rounded-4 position-relative ${logistiqueMode === "gozem" ? "bg-success bg-opacity-10" : "bg-white"}`}
                        style={{
                          cursor: "pointer",
                          transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
                          border:
                      ...
                      >
                        <div className="d-flex align-items-center">
                          <input
                            ...
                            type="radio"
                            value="gozem"
                            checked={logistiqueMode === "gozem"}
                            onChange={(e) => setLogistiqueMode(e.target.value)}
                          />
                          <div className="d-flex flex-column">
                            <span
                              className={`fw-bold fs-6 mb-1 ${logistiqueMode === "gozem" ? "text-success" : "text-dark"}`}
                              style={{ letterSpacing: "-0.2px" }}
                            >
                              Besoin d'un livreur ?
                            </span>
                            <span className="text-muted small fw-medium">
                              Tarif selon votre zone
                            </span>
                          </div>
                        </div>
                        {logistiqueMode === "gozem" && (
                    ...
                    <>
                      <div className="col-md-6">
                        <div className="form-floating">
                          <input
                            type="text"
                            className="form-control shadow-none fw-medium"
                            id="villeInput"
                            placeholder="Ville"
                            value={ville}
                            ...
                            onFocus={(e) =>
                              (e.target.style.borderColor = "#1ab273")
                            }
                            onBlur={(e) =>
                              (e.target.style.borderColor = "#e2e8f0")
                            }
                          />
                          <label htmlFor="villeInput" className="text-muted">
                            Ville
                          </label>
                        </div>
                      </div>
                      <div className="col-md-6">
                        <div className="form-floating">
                          ...
                          />
                          <label htmlFor="quartierInput" className="text-muted">
                            Quartier
                          </label>
                        </div>
                      </div>
                      <div className="col-12">
                        <div className="form-floating">
                          ...
                          ></textarea>
                          <label htmlFor="adresseInput" className="text-muted">
                            Adresse détaillée complète
                          </label>
                        </div>
                      </div>
                    </>
                  ...
                  <div className="col-md-6">
                    <div className="form-floating">
                      ...
                      />
                      <label htmlFor="telInput" className="text-muted">
                        Numéro de Contact (Réception)
                      </label>
                    </div>
                  </div>
                </div>
              ...
              <div style={premiumCardStyle}>
                ...
                <div className="d-flex gap-3 mb-5 p-2 bg-light rounded-4 border border-light shadow-sm">
                  <button
                    type="button"
                    className={`btn flex-fill fw-bolder rounded-3 py-3 border-0 transition-all ${typePaiement === "mobile_money" ? "bg-white shadow text-success" : "text-secondary bg-transparent hover-bg-white"}`}
                    style={{ transition: "all 0.3s ease" }}
                    onClick={() => setTypePaiement("mobile_money")}
                  >
                    <Smartphone size={20} className="me-2" /> Mobile Money
                  </button>
                  <button
                    type="button"
                    className={`btn flex-fill fw-bolder rounded-3 py-3 border-0 transition-all ${typePaiement === "carte" ? "bg-white shadow text-success" : "text-secondary bg-transparent hover-bg-white"}`}
                    style={{ transition: "all 0.3s ease" }}
                ...
                <div
                  className="bg-white rounded-4 border p-4 shadow-sm"
                      ...
                      <div className="d-flex gap-3 mb-4 flex-wrap">
                        {[
                          ...
                          },
                          {
                            ...
                            logo: "/moov_logo.png",
                            color: "#005BAC",
                            desc: "Moov Africa Money",
                          },
                          {
                            name: "Celtis",
                            logo: "/celtis_logo.png",
                            color: "#f97316",
                            desc: "Celtis Mobile",
                          },
                        ].map((op) => (
                          <label
                            key={op.name}
                            className={`form-check flex-fill text-center p-3 rounded-4 cursor-pointer position-relative ${operateur === op.name ? "shadow-sm" : "bg-white"}`}
                            style={{
                              cursor: "pointer",
                              transition: "all 0.2s",
                              border:
                      ...
                      <div className="row g-4">
                        <div className="col-md-6">
                          <div className="form-floating">
                            <input
                              type="text"
                              className="form-control shadow-none fw-medium text-dark fs-6"
                              id="mobileInput"
                              placeholder="Numéro Mobile Money"
                              value={numeroMobile}
                              onChange={(e) => setNumeroMobile(e.target.value)}
                              required={typePaiement === "mobile_money"}
                              style={{
                                borderRadius: "12px",
                                border: "2px solid #e2e8f0",
                                letterSpacing: "1px",
                            ...
                            />
                            <label htmlFor="mobileInput" className="text-muted">
                              Numéro à débiter
                            </label>
                          </div>
                        </div>
                        <div className="col-md-6">
                          <div className="form-floating position-relative">
                            <input
                              type="text"
                              className={`form-control shadow-none fw-medium fs-6 ${confirmNumero && confirmNumero !== numeroMobile ? "is-invalid" : ""}`}
                              id="confirmMobileInput"
                          ...
                          {confirmNumero && confirmNumero !== numeroMobile && (
                            <div className="text-danger small mt-2 fw-medium px-2">
                    ...
                    <div className="animate__animated animate__fadeIn row g-4">
                      <div className="col-12">
                        <div className="form-floating position-relative">
                          <input
                            type="text"
                            className="form-control shadow-none fw-medium fs-6 text-dark"
                            id="carteInput"
                            placeholder="0000 0000 0000 0000"
                            value={numeroCarte}
                            onChange={(e) => setNumeroCarte(e.target.value)}
                            required={typePaiement === "carte"}
                            style={{
                              borderRadius: "12px",
                              border: "2px solid #e2e8f0",
                              letterSpacing: "2px",
                      ...
                      <div className="col-md-6">
                        <div className="form-floating">
                          <input
                            type="text"
                            className="form-control shadow-none fw-medium fs-6 text-center text-dark"
                            id="expInput"
                            placeholder="MM/AA"
                            value={dateExp}
              ...
              <div className="d-flex align-items-center justify-content-center gap-5 mt-2 opacity-75">
                <div className="d-flex align-items-center text-secondary fw-medium">
                  <Lock size={16} className="me-2" /> Certifié SSL 256-bit
                </div>
                <div className="d-flex align-items-center text-secondary fw-medium">
                  <ShieldCheck size={16} className="me-2" /> Protection
                  AgroEscrow
                </div>
              </div>
            </div>
            ...
            <div className="col-lg-5">
              <div className="sticky-top" style={{ top: "100px", zIndex: 10 }}>
                <div
                  style={{ ...premiumCardStyle, padding: "0" }}
                  className="overflow-hidden"
                >
                  <div
                    className="p-4"
                    style={{ backgroundColor: "#0f3a23", color: "#fff" }}
                  >
                    <h5 className="fw-bolder mb-0 d-flex align-items-center gap-2">
                      <CheckCircle2 size={24} className="text-success" /> Résumé
                      de la commande
                    </h5>
                  </div>
                  ...
                  >
                    {panier.map((item) => (
                      <div
                        key={item.panier_key}
                        className="d-flex justify-content-between align-items-center mb-3 pb-3 border-bottom border-light"
                      >
                        <div className="pe-3">
                          <h6
                            className="fw-bolder mb-1 text-dark text-truncate"
                  ...
                  <div className="p-4 bg-white">
                    <label
                      className="d-flex align-items-center gap-3 p-3 bg-white rounded-4 border shadow-sm cursor-pointer mb-4"
                      style={{
                        cursor: "pointer",
                        border: accepted
                          ? "2px solid #1ab273"
                    ...
                    >
                      <input
                        className="form-check-input m-0 shadow-none border-secondary"
                        type="checkbox"
                        checked={accepted}
                      ...
                      <span className="text-secondary small lh-sm fw-medium">
                        J'accepte les <strong>CGU</strong>, la{" "}
                        <strong>Politique de Remboursement</strong> et valide ma
                        commande.
                      </span>
                    </label>

                    {!accepted && (
                      <div className="alert bg-warning bg-opacity-10 text-warning-emphasis py-2 px-3 small fw-bold border-0 d-flex align-items-center rounded-3">
                        <AlertCircle size={16} className="me-2" />
                        Veuillez valider les conditions (CGU).
                      </div>
                    )}
                    {submitError && (
                      <div className="alert bg-danger bg-opacity-10 text-danger py-2 px-3 small fw-bold border-0 d-flex align-items-center rounded-3">
                        <AlertCircle size={16} className="me-2" />
                        {submitError}
                      </div>
                    )}
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className={`btn w-100 rounded-pill py-3 px-4 fw-bolder fs-5 d-flex align-items-center justify-content-center shadow-lg text-white ${accepted ? "btn-success" : "btn-secondary opacity-50"}`}
                      style={{
                        transition: "all 0.3s",
                        transform:
                          accepted && !loading ? "translateY(-2px)" : "none",
                      }}
                      onMouseOver={(e) => {
                        if (accepted && !loading)
                          e.currentTarget.style.boxShadow =
                            "0 15px 30px -10px rgba(26,178,115,0.5)";
                      }}
                      onMouseOut={(e) => {
                        if (accepted && !loading)
                          e.currentTarget.style.boxShadow =
                    ...
                    >
                      {loading ? (
                        <>
                          <span
                            className="spinner-border spinner-border-sm me-3 border-2"
                        ...
                        <>
                          <Lock size={22} className="me-2" /> Payer{" "}
                          {formatPrice(totalPayable)}
                        </>
                      )}
                    </button>
